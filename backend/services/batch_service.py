        ...
        if result.is_error:
            ...
            if doc_type == "PAN":
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not link doc {doc_id} to user automatically")
...
async def process_batch_extraction(batch_id: str,
...
async def initialize_batch(
    batch_id: str, files: List[UploadFile], doc_types: List[str],
    passwords: List[Optional[str]], created_by_user_id: str
    ...
    for file, doc_type, password in zip(files, doc_types, passwords):
        ...
        prepared_files.append(
            (doc_id, file_bytes, mime_type, file.filename, password))

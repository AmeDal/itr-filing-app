...
from backend.auth_deps import require_admin, UserPrincipal, oauth2_scheme
...
from backend.services.auth_service import (create_access_token,
                                           ...
                                           revoke_token)
from backend.services.user_deletion_service import hard_delete_user, bulk_hard_delete_users
...
class ResetPasswordRequest(BaseModel):
    new_password: str = Field(
        ..., 
        min_length=12,
        description="New password must be at least 12 chars and contain uppercase, lowercase, number, and special char."
    )
...
async def list_users(
    ...
    async for doc in cursor:
        ...
        decrypted = await user_service.decrypt_user_doc(doc, skip_fields={"password"})
...
async def change_user_role(
    ...
    if req.role == "user":
        ...
        admin_count = await db.users.count_documents({"role": enc_admin, "is_active": enc_active})
    ...
    result = await db.users.update_one(
        {"_id": user_id},
        {"$set": {"role": await CryptoService.encrypt_deterministic(req.role)}}
    )
...
async def toggle_user_status(
    ...
    await db.users.update_one(
        {"_id": user_id},
        {"$set": {"is_active": await CryptoService.encrypt_deterministic(is_active)}}
    )
    return {"detail": f"User status set to {'active' if is_active else 'inactive'}"}
...
async def delete_user(
    ...
    logger.info(f"[DELETE_ACTION_{action_id}] Request to delete user: {user_id}")
    
    res = await hard_delete_user(user_id)
    
    if not res.get("profile_deleted"):
        logger.warning(f"[DELETE_ACTION_{action_id}] FAILED: User {user_id} not found.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User {user_id} not found or already deleted. (ID: {action_id})"
        )
        
    logger.info(f"[DELETE_ACTION_{action_id}] SUCCESS: User {user_id} deleted.")
    return res
...
async def bulk_delete_users(
    ...
    if not req.user_ids:
        raise HTTPException(status_code=400, detail="No valid user IDs providing for deletion (cannot delete yourself)")
...
async def force_reset_password(
    ...
    if user_id == current_admin.id:
        logger.info(f"Self-reset detected for admin {user_id}. Revoking tokens and cookies.")
